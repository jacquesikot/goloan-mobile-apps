const data = [
  {
    id: 1,
    image: require('../../../assets/images/onboarding1.png'),
    topText: 'Access to Instant Credit',
    bottomText: 'No forms, guarantors, processing fees or tidious sign up process.',
    last: false,
  },
  {
    id: 2,
    image: require('../../../assets/images/onboarding2.png'),
    topText: 'Hassle free payments',
    bottomText:
      'You can easily send money to anyone, pay all your bills, and recharge airtime, and more.',
    last: false,
  },
  {
    id: 3,
    image: require('../../../assets/images/onboarding3.png'),
    topText: 'Earn meaningful points',
    bottomText:
      'For every loan, payment or purchase you make with us, you get points that allow you do more.',
    last: true,
  },
];

export default data;
